...
test('PWA cache includes both approved masters and has a fresh version',()=>{
  const sw=readFileSync(path.join(root,'sw.js'),'utf8');assert.match(sw,/terra-italica-v54/);assert.match(sw,/building_mill_p16\.png/);assert.match(sw,/ui_selection_marker_p16\.png/);
});
