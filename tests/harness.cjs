...
function node(dataset={}){const classes=new Set();return {dataset,style:{},classList:{add:x=>classes.add(x),remove:x=>classes.delete(x),contains:x=>classes.has(x),toggle:(x,on)=>on?classes.add(x):classes.delete(x)},listeners:{},addEventListener(name,fn){(this.listeners[name]??=[]).push(fn)},insertAdjacentHTML(position,html){this.innerHTML=position==='afterbegin'?html+(this.innerHTML||''):(this.innerHTML||'')+html},setAttribute(){},setPointerCapture(){},getBoundingClientRect:()=>({left:0,top:0,width:390,height:600}),getContext:()=>context2d,showModal(){this.open=true},close(){this.open=false;for(const fn of this.listeners.close||[])fn()},querySelectorAll(){return [...(this.innerHTML||'').matchAll(/data-person="([^"]+)"/g)].map(m=>node({person:m[1]}))}}}
