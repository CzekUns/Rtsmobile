...
(() => {
  ...
  Game.prototype.drawBuilding=function(b){if(!RECIPES[b.type]||b.type==='mill'&&b.built&&window.TERRA_P16_SPRITES?.mill?.complete&&window.TERRA_P16_SPRITES.mill.naturalWidth>0)return drawBuilding.call(this,b);const p=this.worldToScreen(b.x*TILE,b.y*TILE),z=this.camera.zoom,c=this.ctx;c.save();c.fillStyle=b.type==='mill'?'#918b72':'#a47854';c.fillRect(p.x-15*z,p.y-13*z,30*z,26*z);c.fillStyle='#3d382f';if(b.type==='mill'){c.fillRect(p.x-2*z,p.y-12*z,4*z,24*z);c.fillRect(p.x-12*z,p.y-2*z,24*z,4*z);}else c.fillRect(p.x-8*z,p.y-3*z,16*z,12*z);if(!b.built){c.fillStyle='#d0b36a';c.fillRect(p.x-15*z,p.y+16*z,30*z*b.progress,3*z);}c.restore();if(this.selected?.id===b.id)this.selectionRing(p.x,p.y,20*z);};
